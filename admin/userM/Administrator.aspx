<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Administrator.aspx.cs" Inherits="Donate.admin.UserManagement.Administrator" %>
...
<body>
    <div class="layui-fluid">
        <form id="form1" runat="server" class="layui-form" lay-filter="component-form-group">
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:DonateConnectionStrings %>" ProviderName="<%$ ConnectionStrings:DonateConnectionStrings.ProviderName %>" SelectCommand="SELECT `user`.*, role.name AS role_name FROM role INNER JOIN `user` ON role.id = `user`.role WHERE  `user`.role = 1 or `user`.role = 2" UpdateCommand="UPDATE `user` SET nick = @nick, gender = @gender, phoneNumber = @phoneNumber, email = @email, role = @role, remarks = @remarks WHERE (uid = @uid)" DeleteCommand="DELETE FROM `user` WHERE (uid = @uid)"></asp:SqlDataSource>
            ...
            <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:DonateConnectionStrings %>" ProviderName="<%$ ConnectionStrings:DonateConnectionStrings.ProviderName %>" SelectCommand="SELECT `user`.uid, `user`.nick, `user`.gender, `user`.imgUrl, `user`.phoneNumber, `user`.email, `user`.role, `user`.remarks, `user`.lastDate, `user`.password, `user`.lastIp, role.name AS role_name FROM role INNER JOIN `user` ON role.id = `user`.role WHERE (`user`.nick LIKE ?) AND (`user`.phoneNumber LIKE ?) AND (`user`.email LIKE ?) AND  `user`.role = 1 or `user`.role = 2">
                <SelectParameters>
